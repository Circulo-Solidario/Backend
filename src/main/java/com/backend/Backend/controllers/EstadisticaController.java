...
public class EstadisticaController {
     ...
     * @return Estadísticas globales de usuarios, productos, proyectos y puntos
     */
    @GetMapping("/global")
     ...
     * @return Estadísticas personales del usuario
     */
    ...
    public ResponseEntity<PersonalEstadisticaResponse> getEstadisticasPersonales(@PathVariable Long id) {
        ...
        resp.setProyectosActivos(proyectosUsuario.stream()
                .filter(p -> p.getEstado() == EstadoProyecto.ACTIVO).count());
        resp.setProyectosFinalizados(proyectosUsuario.stream()
                .filter(p -> p.getEstado() == EstadoProyecto.FINALIZADO_EXITOSO ||
                        p.getEstado() == EstadoProyecto.FINALIZADO_NO_EXITOSO).count());
        resp.setTotalRecaudadoPorProyectos(proyectosUsuario.stream()
                .mapToDouble(Proyecto::getRecaudado).sum());
        ...
        resp.setProductosDisponibles(productosUsuario.stream()
                .filter(p -> p.getEstado() == EstadoProducto.DISPONIBLE).count());
     ...
     * Incluye métricas de proyectos, donaciones, donadores y evolución temporal
     * @param id ID de la organización
     * @return Estadísticas completas de la organización
     */
    @GetMapping("/organizacion/{id}")
    public ResponseEntity<OrganizacionEstadisticaResponse> getEstadisticasOrganizacion(@PathVariable Long id) {
        return ResponseEntity.ok(estadisticasService.obtenerEstadisticasOrganizacion(id));
    }

    /**
     * Obtiene métricas simplificadas de proyectos de una organización
     * @deprecated Usar /api/estadisticas/organizacion/{id} para estadísticas completas
     * @param id ID de la organización
     * @return Métricas básicas de proyectos
     */
    ...
    public ResponseEntity<OrganizacionProyectosMetricsResponse> getMetricsForOrganizacion(@PathVariable Long id) {
        ...
        resp.setProyectosActivos(proyectosOrg.stream()
                .filter(p -> p.getEstado() == EstadoProyecto.ACTIVO).count());
        resp.setProyectosCancelados(proyectosOrg.stream()
                .filter(p -> p.getEstado() == EstadoProyecto.CANCELADO).count());
        resp.setProyectosFinalizadosExitosos(proyectosOrg.stream()
                .filter(p -> p.getEstado() == EstadoProyecto.FINALIZADO_EXITOSO).count());
        resp.setProyectosFinalizadosNoExitosos(proyectosOrg.stream()
